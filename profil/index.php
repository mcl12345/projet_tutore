...
function formulaire_HTML($db_host, $db_name, $db_user, $db_password) {
    ...
    while ($row = $stmt->fetch()) {
      ...
      $stmt_->execute(array($_SESSION["the_id"]));
    ...
    echo "</select>
    <input type='submit' value='Sélectionner' /><br />";
    echo "</form>";
}

// Traitement et Affichage HTML5
if( !empty($_POST["genre_prefere"])) {
    ...
    try {
        ...
        while($row = $stmt->fetch()) {
            ...
            $stmt->execute(array($_POST["genre_prefere"], $_SESSION["the_id"]));
        }
        if(!$is_created) {
